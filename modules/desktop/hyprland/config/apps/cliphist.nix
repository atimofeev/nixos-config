{ lib, pkgs, ... }:
{
  ...
  systemd.user.services.cliphist.Unit.After = lib.mkForce "graphical-session.target";
  systemd.user.services.cliphist-images.Unit.After = lib.mkForce "graphical-session.target";
