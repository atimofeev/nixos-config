...
let
  cfg = config.custom.hardware.peripherals.logitech;
in
{

  options.custom.hardware.peripherals.logitech = {
    logiops.enable = lib.mkEnableOption "Enable logiops service and config";
    logi-bolt-disable-wakeup = lib.mkEnableOption "Prevents Logi Bolt Receiver from waking up the device";
  };

  config = lib.mkMerge [

    (lib.mkIf cfg.logi-bolt-disable-wakeup {
