# NOTE: figure out configuration options
# likely just fully expose devices and folders
{
  pkgs,
  vars,
  lib,
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.setAclForSyncthing = {
      ...
      serviceConfig = {
        ExecStart = "${pkgs.acl}/bin/setfacl -R -m u:${vars.username}:rwx /var/lib/syncthing/";
        Type = "oneshot";
